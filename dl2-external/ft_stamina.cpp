...
void ft_stamina::tick()
{
	...
	if (psm_ptr && Globals::g_pProcess->read< float >( psm_ptr + offsetof(cPlayerStaminaModule, m_flCurrentStamina1) ) < 3.f ) 
	{
		Globals::g_pProcess->write< float >(psm_ptr + offsetof(cPlayerStaminaModule, m_flCurrentStamina1), 3.f);
